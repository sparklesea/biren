...
template <typename DType> struct suDeleter {
  void operator()(DType const *ptr) { suFree((void *)ptr); }
};
...
template <typename DType> RamArray<DType>::RamArray(int _len) {
  ...
  d_array = std::shared_ptr<DType>(nullptr, suDeleter<DType>());
  checkSupaError(suMallocDevice((void **)&d_array, size));
}
...
template <typename DType> void RamArray<DType>::reset() {
  fill_zero_h();
  suMemset(d_array.get(), 0, size);
}

template <typename DType> void RamArray<DType>::upload() {
  checkSupaError(suMemcpy((void *)d_array.get(), (void *)h_array.get(), size,
                            suMemcpyHostToDevice));
}

template <typename DType> void RamArray<DType>::download() {
  checkSupaError(suMemcpy((void *)h_array.get(), (void *)d_array.get(), size,
